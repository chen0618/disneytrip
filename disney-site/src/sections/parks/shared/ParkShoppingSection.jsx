...
import styles from './ParkShoppingSection.module.css';

export default function ParkShoppingSection({ parkName, sectionId, background, onSelectItem }) {
  const [isOpen, setIsOpen] = useState(false);
  const shops = useMemo(
    () => mapShops.filter((s) => s.park === parkName),
    [parkName]
  );

  return (
    <section id={sectionId} className={!isOpen ? styles.sectionCollapsed : ''} style={{ background }}>
